...
// Body: { email, tel, prenom, senderName, content }
...
<head>
  ...
  <style>
    ...
    .msg-box { background: #f5f6fb; border-left: 3px solid #615ca5; border-radius: 0 8px 8px 0; padding: 14px 18px; margin-bottom: 24px; }
...
<body>
  <div class="wrap">
    ...
    <div class="body">
      <p class="intro">Bonjour <strong>${firstName}</strong>,<br>Vous avez reçu un nouveau message de <strong>${sender}</strong> sur votre espace candidat Edumove.</p>
      <div class="msg-box">
        <div class="msg-label">Aperçu du message</div>
        <div class="msg-text">${preview.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</div>
      </div>
      <a href="${siteUrl}/espace.html" style="display:block;background:#ec680a;color:#ffffff !important;text-decoration:none !important;text-align:center;padding:13px 24px;border-radius:10px;font-size:14px;font-weight:600;font-family:'Poppins',Arial,sans-serif;">Consulter mes messages</a>
    </div>
    <div class="footer">
    ...
    try {
      const resp = await fetch('https://api.brevo.com/v3/smtp/email', {
        ...
        body: JSON.stringify({
          ...
          to: [{ email }],
          subject: `💬 ${sender} vous a envoyé un message — Edumove`,
          htmlContent
        })
      });
      const data = await resp.json().catch(() => ({}));
      results.email = resp.ok ? 'sent' : 'error';
      if (!resp.ok) console.error('Brevo notify-student-chat error:', data);
      else console.log(`Student chat email sent to ${email} from ${sender}`);
    } catch (err) {
      results.email = 'error';
  ...
  if (tel && smsEnabled && smsToken) {
    ...
    try {
      const resp = await fetch('https://api.smsfactor.com/send', {
        ...
        body: JSON.stringify({
          sms: {
            message: { text: smsText, pushtype: 'marketing', sender: 'Edumove' },
      ...
      if (!resp.ok || data.status === 0) console.error('SMS Factor error:', data);
      else console.log(`Student chat SMS sent to ${phone} from ${sender}`);
    } catch (err) {
      results.sms = 'error';
